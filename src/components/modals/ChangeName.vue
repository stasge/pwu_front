...
const update = async () => {
    ...
    wrapAsyncCall(async () => {
        await fetchPost('/user/changeName', form)
        userStore.loadUser()
        isDiaShown.value = false
    }, null, "Ім'я форуму успішно змінено")
}
const showDia = (edit: boolean = false, name?: string) => {
    isEdit.value = edit
    reset()
    if (name) {
        form.name = name
        ...
        <template #body>
            <form @submit.prevent="update" class="flex flex-column justify-content-center w-full">
                <div class="field w-full">
                    <label for="name" class="w-full">Нове ім'я</label>
                    <input 
                        v-model="form.name" 
                        id="name" 
                        type="text" 
